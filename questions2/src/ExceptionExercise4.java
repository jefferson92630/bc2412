...
public class ExceptionExercise4 {
  ...
  public static void registerUser(String username, String password,
      String email) {
      try {
        validateUsername(username); // if email checking fail, throw and exit method
        validatePassword(password);
        validateEmail(email);
        System.out.println("User registered successfully: " + username);
      } catch (IllegalArgumentException e) {
        // re-throw, because user no need to care below 3 exception, so new exception is to combine the 3 exceptions.
  ...
  private static void validateUsername(String username) {
    if (username == null || username.isEmpty()) // username point to null
